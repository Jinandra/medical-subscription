...
<div class="right_col" role="main">
  ...
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Edit Category's Media: {{ $category->name }}</h2>
      ...
      <div class="x_content">
        ...
        <tbody>
          @foreach ($media as $medium)
            <tr class="pointer" data-id="{{ $medium->pivot->id }}" data-media-id="{{ $medium->id }}">
              ...
              <td width="15%" class="last_modified">{{ time_ago($medium->pivot->updated_at) }}</td>
              <td class="last" width="15%">
                <button class="btn btn-danger btn-delete"  data-alert-text="Do you want to remove '{{ $medium->title }}' from category?"><i class="fa fa-remove"></i> Delete</button>
              </td>
            </tr>
          @endforeach
        </tbody>
  ...
  $(document).ready(function () {
    $(document.body).on('click', 'tbody .btn-delete', function (e) {
      ...
      if (confirm($($.parseHTML($button.data('alert-text'))).text())) {
        $.ajax({
          url: "{{ url('admin/categories/'.$category->id) }}/media/"+$tr.data('media-id'),
          type: 'DELETE',
          data: {
            _token: "{{ csrf_token() }}"
          },
    ...
    function initSortable () {
      $('tbody').sortable({
        ...
        update: function (e, ui) {
          var sortOrder = getIds('media-id');
          $.ajax({
            type: 'PATCH',
            url:  "{{ route('admin::categories::patchMedia', ['id' => $category->id]) }}",
            data: {
              action: 'sort',
              media_ids: sortOrder,
              _token: "{{ csrf_token() }}"
    ...
    $('#autocomplete').autocomplete({
      ...
      select: function (ul, item) {
        $.ajax({
          type: "POST",
          url: "{{ route('admin::categories::createMedia', ['id' => $category->id]) }}",
          data: {
          ...
          success: function (data) {
            ...
            $('<tr class="pointer ui-sortable-handle" data-id="'+data.pivot_id+'" data-media-id="'+media.id+'">')
              ...
              .append('<td width="15%" class="last_modified">'+data.pivot_updated_at_formatted+'</td>')
              .append('<td class="last" width="15%"><button class="btn btn-danger btn-delete"  data-alert-text="Do you want to remove \''+media.title+'\' from category?"><i class="fa fa-remove"></i> Delete</button>')
              .appendTo('tbody');
            initSortable();
          }
        });
